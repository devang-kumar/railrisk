...
import { wagons, type RiskLevel } from "@/lib/railrisk-data";
...
const riskStyles: Record<RiskLevel, {
  bg: string; border: string; stripe: string; glow: string; text: string;
}> = {
  Critical: {
    bg: "bg-[#ff1e1e]/[0.08]",
    border: "border-[#ff1e1e]/30",
    stripe: "bg-gradient-to-b from-[#ff6b00] to-[#ff1e1e]",
    glow: "shadow-[0_0_12px_rgba(255,30,30,0.3)]",
    text: "text-[#ff1e1e]",
  },
  High: {
    bg: "bg-[#ff6b00]/[0.06]",
    ...
    stripe: "bg-[#ff6b00]",
    glow: "shadow-[0_0_10px_rgba(255,107,0,0.2)]",
    text: "text-[#ff6b00]",
  },
  Medium: {
    bg: "bg-[#fbbf24]/[0.04]",
...
function Overview() {
  ...
  return (
    <>
      ...
      {/* Agent stack — compact horizontal strip */}
      <section className="glass rounded-2xl p-4 mb-8 animate-fade-up stagger-2">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <AgentChip icon={<Timer className="size-3.5" />} name="Delay Detection" status="142 trains" load={72} />
          <AgentChip icon={<Layers className="size-3.5" />} name="Cargo Criticality" status="3 flagged" load={88} hot />
          <AgentChip icon={<TrendingUp className="size-3.5" />} name="Impact Prediction" status="Modeling" load={61} />
          <AgentChip icon={<Zap className="size-3.5" />} name="Action Engine" status="2 dispatched" load={49} />
        </div>
      </section>
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-xs text-white/50 mt-1">
              All delayed wagons, ranked live by criticality — not arrival time.
            </p>
          </div>
          <Link
            to="/wagons"
            className="text-xs text-white/60 hover:text-white inline-flex items-center gap-1 transition-colors duration-300"
          >
              ...
              >
                ...
                <div className="col-span-1 shrink-0">
                  <div className={`size-10 rounded-xl flex items-center justify-center text-xs font-bold border ${
                    w.risk === "Critical" ? "glass-ember" :
                    w.risk === "High" ? "bg-[#ff6b00]/10 border-[#ff6b00]/25" :
                    "glass"
                  }`}>
                    {w.id}
                  </div>
      ...
      {/* Risk distribution + escalation — side by side */}
...
function AgentChip({ icon, name, status, load, hot }: {
  icon: React.ReactNode; name: string; status: string; load: number; hot?: boolean;
}) {
  return (
...
function RiskDistribution() {
  const buckets = [
    ...
    { label: "Medium", count: 1, color: "#fbbf24", pct: 14 },
    { label: "Low", count: 2, color: "#34d399", pct: 29 },
  ];
  return (
